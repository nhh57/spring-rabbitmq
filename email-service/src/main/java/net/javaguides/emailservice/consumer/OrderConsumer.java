...
public class OrderConsumer {
    ...
    @RabbitListener(queues = "${rabbitmq.dlq.email.name}")
    public void consumerToQueueFailed(OrderEvent event, Channel channel, Message message) throws IOException {
        try {
            System.out.println("HOT FIX in email Received message: " + event.toString());
            // Xử lý thông điệp theo logic của bạn
            channel.basicAck(message.getMessageProperties().getDeliveryTag(), false);
        } catch (Exception e) {
            System.err.println("Failed  to process in email message: " + e.getMessage());
            channel.basicNack(message.getMessageProperties().getDeliveryTag(), false, false);
    ...
    public void consumePubSub(OrderEvent event, Channel channel, Message message) throws IOException {
        try {
            ...
            LOGGER.info(String.format("consumePubSub -- Order event received in email service => %s", event.toString()));
            channel.basicAck(message.getMessageProperties().getDeliveryTag(), false);
        } catch (IllegalArgumentException e) {
            System.out.println("consumePubSub email Error occurred: " + e.getMessage() + ". Tiếp tục xử lý các process khác.");
            channel.basicNack(message.getMessageProperties().getDeliveryTag(), false, false);
        }
    }
}
